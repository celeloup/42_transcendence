...
export default class ChannelService {
	...
	async isAuthorized(channel_id: number, user: User) {
		const channel = await this.getAllInfosByChannelId(channel_id);
		if ((channel.members.findIndex(element => element.id === user.id)) === -1)
			return false;
		if ((channel.banned.findIndex(element => element.id === user.id)) !== -1)
			return false;
		if (channel.next_unmute_date && channel.next_unmute_date < Date.now()
			&& ((await this.muteStillUpToDate(channel, user.id))))
			return false;
		return true;
	}
